...
b4obs = URIRef("https://ci.mines-stetienne.fr/kg/itmfactory/vl10/B4#this")
b4target = URIRef("https://ci.mines-stetienne.fr/kg/itmfactory/vl10/B4#AxeZ_Fdc_Bas_B4")
...
g.add((Switch, sosa.observes, Literal("It indicates belt reaches to bottom point on the Z- Axis on VL10 machine of the IT'm factory.", lang=("en"))))
...
g.add((machine, dct.title, Literal("Z-Axis bottom limit Status")))
g.add((machine, dct.description, Literal("Indicates the status of the Z axis at the bottom limit point on VL10 machine of the IT'm factory. Normally the switch is in close and the status is true, if belt reaches to bottom limit point then the switch is opened and the status is false.", lang=("en"))))
...
g.add((Operation, hctl.hasTarget, b4target))
g.add((machine, td.isObservable, Literal("False")))
g.add((machine, td.name, Literal("AxeZ_Fdc_Bas_B4")))

print(g.serialize(r"C:\Users\Siva Ratnam Pachava\OneDrive\Desktop\InternshipM1\VL10\B4.ttl" , format='ttl'))