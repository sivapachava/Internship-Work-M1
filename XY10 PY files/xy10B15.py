...
g.add((Switch, sosa.observes, Literal("It indicates belt reaches to front limit point on the X- Axis on XY10 machine of the IT'm factory", lang=("en"))))
...
g.add((machine, dct.title, Literal("XAxisFrontLimit")))
g.add((machine, dct.description, Literal("Indicates the status of the X axis on XY10 machine of the IT'm factory at the front limit point. Normally the switch is in close and the status is true, if belt reaches to front limit point then the switch is opened and the status is false.", lang=("en"))))
...
g.add((Operation, hctl.hasTarget, Literal("https://ci.mines-stetienne.fr/simu/packagingWorkshop/properties/AxeX_Fdc_Avant_B15")))
g.add((machine, td.isObservable, Literal("False")))
g.add((machine, td.name, Literal("AxeX_Fdc_Avant_B15")))

print(g.serialize(r"C:\Users\Siva Ratnam Pachava\OneDrive\Desktop\InternshipM1\XY10\B15.ttl" , format='ttl'))