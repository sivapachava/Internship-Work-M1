...
g.add((Switch, sosa.observes, Literal("It indicates the belt is reaches to the intial position on the X axis on XY10 machine of the IT'm factory", lang=("en"))))
...
g.add((machine, dct.title, Literal("XAxis Reference")))
g.add((machine, dct.description, Literal("Indicates the belt is reaches to the intial position on the X axis on XY10 machine of the IT'm factory. Normally the switch is in open and the status is false, if belt reaches to intial position then the switch is closed and the status is true.", lang=("en"))))
...
g.add((Operation, hctl.hasTarget, Literal("https://ci.mines-stetienne.fr/simu/packagingWorkshop/properties/AxeX_Ref_B20")))
g.add((machine, td.isObservable, Literal("False")))
g.add((machine, td.name, Literal("AxeX_Ref_B20")))

print(g.serialize(r"C:\Users\Siva Ratnam Pachava\OneDrive\Desktop\InternshipM1\XY10\B20.ttl" , format='ttl'))